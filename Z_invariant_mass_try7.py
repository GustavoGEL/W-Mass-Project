...
# 2) Using numpy to make calculations since it is more efficient                                                                          #
# 3) Storing the output histogram values in a summary file, so that the plot can be edited without all the calculations                   #
# 4) Create a new script (hist_maker.py) with the necessary directory paths, which uses matplotlib.pyplot to plot and save the histogram  #
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
# Get data from file (Using Bisanne)
...
# Try 6 values
# e_min, e_max, use_range = 70, 110, True  # [GeV]  # 1st iteration
# e_min, e_max, use_range = 80, 100, True  # [GeV]  # 2nd iteration
# e_min, e_max, use_range = 85, 95, True  # [GeV]  # 3rd iteration

# e_min, e_max, use_range = 70, 110, True  # [GeV]  # 1st iteration
# e_min, e_max, use_range = 80, 100, True  # [GeV]  # 2nd iteration
e_min, e_max, use_range = 85, 95, True  # [GeV]  # 3rd iteration
# e_min, e_max, use_range = 90, 93, True  # [GeV]  # 4th iteration
nth_iter = 3
...
for entry in tree:
    ...
    # print(f"Total energy (method 6): {E_tot} [GeV]")
    # print(f"Rest mass of Z boson (method 6): {Z_rest_mass} [GeV]")
...
n_bins = 100
plot_ttl = r'$Z^{0}$ rest mass - iteration '+str(nth_iter)
...
# minor_x_tick_step = 250  # 1st iter
# minor_x_tick_step = 1.25  # 2nd iter
minor_x_tick_step = 1  # 3rd iter
# minor_x_tick_step = 0.25  # 4th iter

# minor_y_tick_step = 0.005  # 1st iter
# minor_y_tick_step = 0.01  # 2nd iter
minor_y_tick_step = 0.01  # 3rd iter
# minor_y_tick_step = 0.05  # 4th iter

spine_lines_width = 2
tick_width = spine_lines_width
maj_tick_lngth = 8
min_tick_lngth = maj_tick_lngth*0.5
fig_size_x_y = (12, 7)  # All energies: (14, 7)  # Ranges 1: (12, 7)

if out_plot_type == "phys_rev":
    fig_all_nam = f"rest_mass_hist_{tot_vals}_all_calculated_phys_rev_{str(nth_iter)}_iter.png"
    fig_range_nam = f"rest_mass_hist_{tot_vals}_selected_range_phys_rev_{str(nth_iter)}_iter.png"
    sum_fig_nam = f"rest_mass_hist_{tot_vals}_summary_phys_rev_{str(nth_iter)}_iter.txt"
    sum_fig_all_dat = f"rest_mass_hist_{tot_vals}_all_calc_dat_phys_rev_{str(nth_iter)}_iter.txt"
    sum_fig_range_dat = f"rest_mass_hist_{tot_vals}_range_calc_dat_phys_rev_{str(nth_iter)}_iter.txt"
elif out_plot_type == "personal":
    fig_all_nam = f"rest_mass_hist_{tot_vals}_all_calculated_personal_{str(nth_iter)}_iter.png"
    fig_range_nam = f"rest_mass_hist_{tot_vals}_selected_range_personal_{str(nth_iter)}_iter.png"
    sum_fig_nam = f"rest_mass_hist_{tot_vals}_summary_personal_{str(nth_iter)}_iter.txt"
    sum_fig_all_dat = f"rest_mass_hist_{tot_vals}_all_calc_dat_personal_{str(nth_iter)}_iter.txt"
    sum_fig_range_dat = f"rest_mass_hist_{tot_vals}_range_calc_dat_personal_{str(nth_iter)}_iter.txt"
else:
...
# Define axis
fig, axes = plt.subplots(1, 1, figsize=fig_size_x_y)  # , figsize=(10,7), dpi=600)  # Define layout, numbers represent: (number of rows, number of columns?)
...
"""print(f"Gotten bins: {avg_bin_vals}")
print(f"len: {len(avg_bin_vals)}")
print(f"Gotten n_counts: {n_counts}")
print(f"len: {len(n_counts)}")"""

data_mean = np.average(avg_bin_vals, weights=n_counts)
data_varn = np.average((avg_bin_vals-data_mean)**2, weights=n_counts)
data_std = np.sqrt(data_varn)

print(f"Calculated n_counts: {n_counts}")
print(f"Calculated bins: {bins}")

print(f"Mean Rest Mass: {data_mean} [GeV]")
print(f"Standard Deviation: {data_std} [GeV]")

if out_plot_type == "personal":
    ax.set_title(plot_ttl)
...
if out_plot_type == "personal":
    ...
    # Try 6 values
    # ax.text(4000, 0.00150, txt_lbl_1)  # Using for all energies range
    # ax.text(97, 0.035, txt_lbl_1)  # Using for 70-110 GeV, 1st iter
    # ax.text(80, 0.07, txt_lbl_1)  # Using for 80-100 GeV, 2nd iter
    # ax.text(85, 0.13, txt_lbl_1)  # Using for 85-95 GeV, 3rd iter
    
    # ax.text(4000, 0.00150, txt_lbl_1)  # Using for all energies range
    # ax.text(97, 0.035, txt_lbl_1)  # Using for 70-110 GeV, 1st iter
    # ax.text(80, 0.07, txt_lbl_1)  # Using for 80-100 GeV, 2nd iter
    ax.text(85, 0.13, txt_lbl_1)  # Using for 85-95 GeV, 3rd iter
    # ax.text(90, 0.42, txt_lbl_1)  # Using for 90-93 GeV, 4th iter
...
if run_mode == "save and show" or run_mode == "save":
    ...
    if use_range:
        ...
        with open(f"FinalFigures/{sum_fig_range_dat}", "w") as fl:
            ...
            fl.write(f"fig_size_x_y: {str(fig_size_x_y)}\n")
            fl.write(f"Iteration number: {nth_iter}\n")
...
# Currently doing:
# 1) Splitting program into 1) Calculator and saver of all data energy output values, 2) Optimizer, i.e. iterator which narrows down the 
