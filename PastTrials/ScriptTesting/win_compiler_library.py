...
def compile_c(fil_dir_abs, c_fl_nm, exe_fl_nm):
...
def compile_c_uni_settings(fil_dir_abs, c_fl_nm, exe_fl_nm):
    """Function that compiles a .c file into a .exe file. NOTE: Check that gcc is already installed and working before
    using this function. Otherwise, it is not going to work"""
...
def run_c_file(fil_dir_abs, exe_fl_nm):
...
def debug_c_compilation(fil_dir_abs_pth, c_fil_nm, exe_fil_nm):
