...
out_img_fl_nm = 'muPT_distribution_bare_and_photons'
out_meta_fl_nm = 'muPT_distribution_bare_and_photons_metadata'
running_script_name = '"W_mass_calculator (2_photon_pt).py"'

tree_fill_var = "mu_truth_bare_pt"
tree_lead_photon_pt = "mu_truth_leading_photon_pt" # Just 1 photon case
tree_second_lead_photon_pt  = "mu_truth_subleading_photon_pt"
# Also need: "mu_truth_subleading_photon_pt"
# Define constants
...
parms_fit = 2
w_mass_mev, w_boson_width_mev = 80379, 2070
...
# The following condition accounts for all events, currently trying to get individual plots for 0, 1 and 2 photons (XOR) to see dependence on number of photons
condition_dat_arr = [f'(1.0+({tree_lead_photon_pt}/{tree_fill_var})+({tree_second_lead_photon_pt}/{tree_fill_var}))*int((mu_truth_leading_photon_pt == 0 && mu_truth_subleading_photon_pt == 0) || (mu_truth_leading_photon_pt > 0 && mu_truth_subleading_photon_pt <= 0) || (mu_truth_leading_photon_pt <= 0 && mu_truth_subleading_photon_pt > 0) || (mu_truth_leading_photon_pt > 0 && mu_truth_subleading_photon_pt > 0))']
n_photons = [0]

# Condition to calculate the W mass (using bare pt of muon)
# Idea: find percentage of contribution from (photon/bare muon) * condition to account for each weight
# f'(1.0+({tree_photon_pt}/{tree_fill_var}))'  # '*int(mu_truth_leading_photon_pt == 0 && mu_truth_subleading_photon_pt == 0)'  # 0 photons

hist_axis = ["lin", "lin"]  # [x, y] log: logarithmic, lin: linear
data_x_lbl, data_y_lbl, data_title_lbl = "muon PT [GeV]", "Normalised counts", "P_{T,#mu} distribution (bare+all_photons)"
chi_square_x_lbl, chi_square_y_lbl, chi_square_title_lbl = "Hypothesis mass [GeV]", "$χ^{2}$ value", "$χ^{2}$ distribution of fits"
optim_x_lbl, optim_y_lbl, optim_title_lbl = "muon PT [GeV]", "Normalised counts", "Optimal W fit (bare+all_photons)"
...
plt.scatter(real_chi_unit_roots,fit_vls(real_chi_unit_roots), c='green', label="Mass value limits, $χ^{2}$ = 1", zorder=1)
...
plt.plot(hyp_mass_rng,chi_max_vls_rng, linestyle='--', c='black', label="$χ^{2}$ = 1", zorder=0)
if run_mode == "Summary":
...
""")
    ...
    \\begin{{figure}}[tb]
		...
		\\caption{{\\small Data and optimum fit with $\\chi^{2} = {optim_model_chi_square}$. Used the hypothesis mass of {real_roots[0]}$\pm${max_diff} $[GeV/c^{{2}}]$. }}
