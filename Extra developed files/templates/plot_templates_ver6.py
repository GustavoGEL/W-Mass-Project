...
plt.title("$χ^{2}$ distribution of fits")
plt.scatter(hyp_mass_rng, chi_square_arr, marker='o', c='red', label="$χ^{2}$ values")
plt.plot(hyp_mass_rng,fit_vls(hyp_mass_rng), c='black', label="$χ^{2}$ parabola fit")
plt.xlabel("Hypothesis mass [$\\frac{GeV}{c^{2}}$]")
...
hist_optim.GetXaxis().SetTitle("Energy [#frac{GeV}{c^{2}}]")  # Fraction in label example: "#frac{A}{B}", using latex syntax
...
""")
    ...
    \\begin{{figure}}[tb]
		...
		\\caption{{\\small Data and optimum fit with $\\chi^{2} = {optim_model_chi_square} [GeV/c^{{2}}]$. Used the hypothesis mass of {real_roots[0]} $[GeV/c^{{2}}]$. }}
...
sh_fl_content = """Already using commands from .py script"""
