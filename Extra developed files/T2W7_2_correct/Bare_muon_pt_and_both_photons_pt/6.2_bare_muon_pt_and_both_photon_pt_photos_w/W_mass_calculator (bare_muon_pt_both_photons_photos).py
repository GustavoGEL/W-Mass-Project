...
out_img_fl_nm = 'muPT_distribution_2_photons_photos'
out_meta_fl_nm = 'muPT_distribution_2_photons_photos_metadata'
running_script_name = '"W_mass_calculator (bare_muon_pt_both_photons_photos).py"'
...
condition_dat_arr = [f'(1.0+({tree_lead_photon_pt}/{tree_fill_var})+({tree_second_lead_photon_pt}/{tree_fill_var}))*{weigth_var}*int(mu_truth_leading_photon_pt > 0 && mu_truth_subleading_photon_pt > 0)',]
n_photons = [2]

hist_axis = ["lin", "lin"]  # [x, y] log: logarithmic, lin: linear
data_x_lbl, data_y_lbl, data_title_lbl = "muon PT [GeV]", "Normalised counts", "Bare P_{T,#mu} + P_{T,2 photons} * photos distribution"
chi_square_x_lbl, chi_square_y_lbl, chi_square_title_lbl = "Hypothesis mass [GeV]", "$χ^{2}$ value", "$χ^{2}$ distribution of fits"
optim_x_lbl, optim_y_lbl, optim_title_lbl = "muon PT [GeV]", "Normalised counts", "Optimal W fit ((Bare P_{T,#mu} + P_{T,2 photons}) * photos)"
