...
out_img_fl_nm = 'muPT_bare_distribution_2_photons_photos_real_1'
out_meta_fl_nm = 'muPT_bare_distribution_2_photons_photos_real_1_metadata'
running_script_name = '"Bare_2_photons_photos_real_1.py"'
# Data = bare, Model = born

tree_data_fill_var = "mu_truth_bare_pt"
tree_model_fill_var = "mu_truth_bare_pt"
tree_lead_photon_pt = "mu_truth_leading_photon_pt*(1+0.01*gaussian_random_number)" # Just 1 photon case
tree_second_lead_photon_pt  = "mu_truth_subleading_photon_pt*(1+0.01*gaussian_random_number)"
weigth_var = "photos_fsr_weight" # Not using weights: "photos_fsr_weight", "herwig_fsr_weight"
# Define constants
...
condition_dat_arr = [f'((1.0+({tree_lead_photon_pt}/{tree_data_fill_var})+({tree_second_lead_photon_pt}/{tree_data_fill_var}))*{weigth_var}*int(mu_truth_leading_photon_pt > 0 && mu_truth_subleading_photon_pt <= 0))+((1.0+({tree_lead_photon_pt}/{tree_data_fill_var})+({tree_second_lead_photon_pt}/{tree_data_fill_var}))*{weigth_var}*int(mu_truth_leading_photon_pt <= 0 && mu_truth_subleading_photon_pt > 0))']
condition_mod_arr = [f'((1.0+({tree_lead_photon_pt}/{tree_model_fill_var})+({tree_second_lead_photon_pt}/{tree_model_fill_var}))*{weigth_var}*int(mu_truth_leading_photon_pt > 0 && mu_truth_subleading_photon_pt <= 0))+((1.0+({tree_lead_photon_pt}/{tree_model_fill_var})+({tree_second_lead_photon_pt}/{tree_model_fill_var}))*{weigth_var}*int(mu_truth_leading_photon_pt <= 0 && mu_truth_subleading_photon_pt > 0))']
n_photons = [2]

hist_axis = ["lin", "lin"]  # [x, y] log: logarithmic, lin: linear
data_x_lbl, data_y_lbl, data_title_lbl = "muon PT [GeV]", "Normalised counts", "Bare P_{T,#mu} + P_{T,2photons} photos distribution"
chi_square_x_lbl, chi_square_y_lbl, chi_square_title_lbl = "Hypothesis mass [GeV]", "$χ^{2}$ value", "$χ^{2}$ distribution of fits"
optim_x_lbl, optim_y_lbl, optim_title_lbl = "muon PT [GeV]", "Normalised counts", "Optimal W fit (Bare P_{T,#mu} + P_{T,2photons} photos)"
