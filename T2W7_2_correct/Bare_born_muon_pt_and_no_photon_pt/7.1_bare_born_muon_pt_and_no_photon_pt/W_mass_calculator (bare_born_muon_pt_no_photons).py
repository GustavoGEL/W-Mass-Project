...
out_img_fl_nm = 'muPT_bare_born_distribution_0_photon'
out_meta_fl_nm = 'muPT_bare_born_distribution_0_photon_metadata'
running_script_name = '"W_mass_calculator (bare_born_muon_pt_no_photons).py"'
...
condition_dat_arr = [f'int(mu_truth_leading_photon_pt == 0 && mu_truth_subleading_photon_pt == 0)',]
condition_mod_arr = [f'int(mu_truth_leading_photon_pt == 0 && mu_truth_subleading_photon_pt == 0)',]
n_photons = [0]

hist_axis = ["lin", "lin"]  # [x, y] log: logarithmic, lin: linear
data_x_lbl, data_y_lbl, data_title_lbl = "muon PT [GeV]", "Normalised counts", "Bare P_{T,#mu} + P_{T,no photons} distribution"
chi_square_x_lbl, chi_square_y_lbl, chi_square_title_lbl = "Hypothesis mass [GeV]", "$χ^{2}$ value", "$χ^{2}$ distribution of fits"
optim_x_lbl, optim_y_lbl, optim_title_lbl = "muon PT [GeV]", "Normalised counts", "Optimal W fit (Bare P_{T,#mu} + P_{T,no photons})"
